//
//  ALDotMatrixMeterScanViewController.m
//  AnylineExamples
//
//  Created by Philipp Müller on 01/12/16.
...
NSString * const kDotMatrixMeterScanLicenseKey = kDemoAppLicenseKey;

// The controller has to conform to <AnylineEnergyModuleDelegate> to be able to receive results
@interface ALDotMatrixMeterScanViewController ()<AnylineEnergyModuleDelegate, AnylineNativeBarcodeDelegate>
...
@implementation ALDotMatrixMeterScanViewController
...
- (void)viewDidLoad {
    ...
    BOOL success = [self.anylineEnergyView setupWithLicenseKey:kDotMatrixMeterScanLicenseKey delegate:self error:&error];
    ...
    success = [self.anylineEnergyView setScanMode:ALDotMatrixMeter error:&error];
    if( !success ) {
        // Something went wrong. The error object contains the error description
