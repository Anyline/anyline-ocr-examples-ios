...
#import "ALUniveralIDResultHelper.h"
...
- (BOOL)appendValue:(NSString * _Nullable)value
    ...
    [self addObject:[ALResultEntry withTitle:key value:value shouldSpellOutValue:spellOut]];
    return YES;
}
...
- (NSString *)formattedDate {
    return [ALUniveralIDResultHelper dateStringFromString:self];
}
...
- (NSArray<ALResultEntry *> *)resultEntryList {
    ...
    NSArray <NSString*> *fieldNames = [ALUniveralIDResultHelper fieldNames];
    [fieldNames enumerateObjectsUsingBlock:^(NSString *fieldNameWithUnderscore,
        ...
        if (![fieldName localizedCaseInsensitiveContainsString:@"formatted"]) {
            ALResultEntry *entry;
            entry = [ALUniveralIDResultHelper resultEntryFromFieldName:fieldName value:field.latinText];
            if (entry) {
                [resultData addObject:entry];
            }
            entry = [ALUniveralIDResultHelper resultEntryFromFieldName:fieldName value:field.arabicText scriptCode:@"ara"];
            if (entry) {
                [resultData addObject:entry];
            }
            entry = [ALUniveralIDResultHelper resultEntryFromFieldName:fieldName value:field.cyrillicText scriptCode:@"cyr"];
            if (entry) {
                [resultData addObject:entry];
            }
        } else {
            NSString *fieldNameTitleCase = [ALUniveralIDResultHelper camelCaseToTitleCaseModified:fieldName];
    ...
    return [ALUniveralIDResultHelper sortedResultListFrom:resultData];
}
        ...
        if (aamvaData) { // PDF417 special standard
            ...
            for (NSString *field in fields) {
                ...
                if ((value = [aamvaData valueForKey:field])) {
                    ...
                    NSString *betterFieldName = [ALUniveralIDResultHelper camelCaseToTitleCaseModified:[field stringByTrimmingCharactersInSet:underscores]];
