...
# undef any
# pragma clang attribute push(__attribute__((external_source_symbol(language="Swift", defined_in="Anyline",generated_declaration))), apply_to=any(function,enum,objc_interface,objc_category,objc_protocol))
...
/// Weakly held. Reads and writes are synchronized so the host may set or
/// clear the delegate from any thread (e.g. on the main thread during
/// ScanView teardown) while <code>log()</code> reads it from scan worker threads
/// (MSDK-1524). NOTE: <code>loggedUIFeedbackMessage</code> may be invoked concurrently
/// from multiple threads, so delegate implementations must be thread-safe.
@property (nonatomic, strong) id <ALUIFeedbackLoggerDelegate> _Nullable delegate;
